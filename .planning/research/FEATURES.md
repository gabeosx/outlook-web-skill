...
1. **Actual ARIA attributes on message list rows** — snapshot a real Outlook session and audit what `aria-label` values appear on `[role="listitem"]` elements. They encode subject, sender, date, read status, flag status in a combined string like "Subject: Q3 Report, From: Alice Smith, Received: Thursday 3:42 PM, Unread, Flagged."
