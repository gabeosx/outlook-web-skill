...
- **Colleague names and email addresses** (lines 325-328): `External Contact contact@example.com`, `Colleague A colleague@example.com` — third-party individuals who did not consent to having their contact details committed to a repository.
- **Search history** (lines 323-328): `[search term redacted]`, `[search term redacted]`, `[search term redacted]` — reveals business relationships and vendor communications.
- **Email subjects with business context** (lines 263-288): `"[Subject Redacted]"`, `"[Subject Redacted]"` — internal business communications.
- **Financial account activity** (lines 424, 434-448): `"Financial Services Sender A new confirmation has been posted to your account"` — reveals a financial services relationship and account activity.
- **Health-related communication** (lines 383-408): The External Org fundraising fundraising email (sender name, job title, subject) is sensitive health-adjacent correspondence.
- **Internal access control disclosure** (lines 427-429): `"User has requested access to [Subject Redacted]"` — reveals the account owner's full name and an internal security monitoring system.
- **Live direct-link URL to a specific email** (lines 108, 214, 372): `https://mail.example.com/mail/id/AAkALg...<REDACTED_ITEM_ID>` — a valid reference to a real message in the user's live inbox.
- **Real Exchange ItemIDs** (lines 95-96, 298): The base64 item IDs (`AAkALg...<REDACTED_ID>`, `AQAAAAAAJuqM...<REDACTED>`) are real identifiers for messages in the live mailbox.
