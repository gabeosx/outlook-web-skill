...
| 2 | Stable message ID attribute identified with concrete example value | VERIFIED | Two extraction methods documented with concrete values. URL method: `/mail/id/AAkALg...<REDACTED_ITEM_ID>` (line 96). DOM method: `option` element `id` attribute `AQAAAAAAJuqM...<REDACTED>` with eval code example (lines 113-124). Both `confirmed-live`. |
