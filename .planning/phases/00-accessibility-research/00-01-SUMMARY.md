...
- The `has:attachment` message opened for capture 8 (Financial Services Sender) contained only inline images, not file-download attachments. The attachment reading pane ARIA structure remains unresolved. This is documented in the reference document as `unresolved` with a recommendation for Phase 3.
...
- **Phase 1 (auth):** Use `https://mail.example.com/mail/inbox` as navigation target; check for redirect to `login.microsoftonline.com` to detect auth state
