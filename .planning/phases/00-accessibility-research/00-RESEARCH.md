...
A live agent-browser session was already connected to `https://mail.example.com/mail/inbox` during research. A full `snapshot -i` was captured, yielding confirmed-live ARIA values for the inbox view including the message list container, individual message options, reading pane, and search box. This eliminates the largest uncertainty — the actual ARIA attribute names — for the inbox view.
...
OUTLOOK_URL="https://mail.example.com"  # override with $OUTLOOK_URL env var
BASE_URL="${OUTLOOK_URL:-https://mail.example.com}"
...
agent-browser --auto-connect open https://mail.example.com/mail/inbox
agent-browser snapshot -i
```
...
```bash
# Source: .agents/skills/agent-browser/references/authentication.md
agent-browser --profile "$OUTLOOK_BROWSER_PROFILE" open https://mail.example.com/mail/inbox
agent-browser snapshot -i
```
...
These values were extracted from a live `agent-browser snapshot -i` against `https://mail.example.com/mail/inbox` on 2026-04-10.
...
```
- option "Unread Marked as high priority by Copilot External Contact [Subject Redacted] 
...
- main "Reading Pane" [ref=e14]
  - heading "[Subject Redacted]" [level=3, ref=e59]
  - button "High priority, A request is made for you..." [ref=e60]
    - button "Dismiss summary" [ref=e87]
  - heading "[Subject Redacted]" [level=3, ref=e88]
  - button "Opens card for External Contact" [ref=e113]
  - heading "From: External Contact" [level=3, ref=e89]
    - button "From: External Contact" [ref=e91]
  - heading "To: User, Colleague C" [level=3, ref=e92]
    - button "User" [ref=e143]
    - button "Colleague C" [ref=e144]
  - heading "Cc: Colleague B" [level=3, ref=e93]
    - button "Colleague B" [ref=e145]
...
| Explanation | Focused Inbox may be disabled on this Company account, or the inbox was displaying "All" messages with no split | confirmed-live (absence) |
...
| Example ID (obfuscated) | `AAkALg...<REDACTED_ITEM_ID>` | confirmed-live |
...
**Finding:** This Outlook tenant (`mail.example.com`) uses URL routing that keeps the URL at `mail/` after search — the search query is maintained in SPA state, not in the URL. The deeplink format is documented but produced a persistent loading state, possibly due to tenant routing configuration. The capture script should navigate using the combobox workflow, not the deeplink URL.
...
**Why it happens:** Custom domain (`mail.example.com`) has specific routing; deeplink format may be version-dependent.
...
**Why it happens:** Managed devices (Company, enterprise M365) enforce device compliance checks; automated Chrome for Testing fails these.
...
**Warning signs:** Redirect to `login.microsoftonline.com` or `mail.example.com/auth/` rather than inbox. [CITED: REQUIREMENTS.md CFG-03, AUTH-02]
...
# Example raw URL:
# https://mail.example.com/mail/inbox/id/AAkALg...<REDACTED_ITEM_ID>
```
...
| A2 | `deeplink/search?query=` works on `outlook.office.com` even though it failed for `mail.example.com` tenant | Search URL Format | Medium — if also broken on office.com, URL-based search is not viable at all |
...
3. **Is Focused Inbox enabled or permanently disabled on this Company account?**
...
5. **Does `deeplink/search?query=` work on `outlook.office.com` (non-custom domain)?**
   - What we know: It failed on `mail.example.com`; URL format is documented in Microsoft Q&A.
...
| Outlook web access (`mail.example.com`) | All captures | Yes (active session confirmed) | — | — |
...
- Live `agent-browser snapshot -i` session against `https://mail.example.com/mail/inbox` — 2026-04-10 (inbox view, message list, reading pane, search box)
...
- [Get deep link to Outlook email](https://joshuachini.com/2024/11/09/get-deep-link-to-outlook-email/) — ItemID deeplink format
