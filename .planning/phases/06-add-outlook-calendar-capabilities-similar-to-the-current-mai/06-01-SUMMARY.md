...
- **Issue:** `OUTLOOK_BASE_URL.replace('/mail/', '/calendar/')` fails because the base URL is `https://myemail.accenture.com` (no `/mail/` segment). The direct navigation attempt returned to `/mail/` instead.
...
- **Fix:** Documented the popup card structure in full (confirmed-live capture of "AIG passwordless assessment daily" event). Plan 06-02 implementation must detect this popup by finding the `button "View event"` button and traversing up to the containing generic.
