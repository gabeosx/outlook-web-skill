...
key-decisions:
  ...
  - "Location accumulation: multiple address parts (e.g., '123 Main Street, Anytown, NY 10001') collapsed until 'By ' is reached"
...
- **Organizer firstname heuristic:** After "By Lastname", if the next split-part is a single word not in TERMINAL_TOKENS, it is merged as the firstname (e.g., "By Smith" + "Jordan" → "Org, Alpha"). This handles the dominant "Lastname, Firstname" format seen in ARIA exploration.
- **Location accumulation:** For physical addresses with multiple comma-separated parts (e.g., "123 Main Street, Anytown, NY 10001"), parts are accumulated until "By " or a terminal token is encountered.
...
- **Issue:** Split on ", " breaks "By Org, Alpha" into ["By Smith", "Jordan"]; naive parsing only captured last name
...
- **Verification:** T1 test — organizer correctly parses as "Org, Alpha"
