...
| No real user data (company/company names) | grep for company identifiers | CLEAN — no matches | PASS |
...
| references/digest-signals.md | 83-87 | Hardcoded fallback bulk_patterns in lib/digest.js (ezcater, company-newsletter) not documented — identified in 05-REVIEW.md WR-03 | Info | No user-facing confusion unless a message from one of these senders gets a "bulk" signal and the user asks why — the documented patterns won't explain it. |
