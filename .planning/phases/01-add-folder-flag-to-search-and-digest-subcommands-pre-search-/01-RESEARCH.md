...
- `/Users/username/outlook-web-skill/references/outlook-ui.md` — ARIA capture 1: `treeitem` structure in Navigation pane (direct codebase inspection)
- `/Users/username/outlook-web-skill/lib/search.js` — existing batch pattern, `parseArgs`, `executeComboboxSearch` (direct codebase inspection)
- `/Users/username/outlook-web-skill/lib/digest.js` — existing `runDigest` batch pattern (direct codebase inspection)
- `/Users/username/outlook-web-skill/lib/run.js` — `runBatch` implementation (direct codebase inspection)
- `/Users/username/outlook-web-skill/policy-search.json` — confirmed `click` is allowed (direct codebase inspection)
- `/Users/username/outlook-web-skill/.agents/skills/agent-browser/SKILL.md` — `find role treeitem click --name` pattern (direct codebase inspection)
- `/Users/username/outlook-web-skill/.planning/STATE.md` — Phase 2 decision: snapshot must be last in batch; page resets to about:blank after batch exit (direct codebase inspection)
