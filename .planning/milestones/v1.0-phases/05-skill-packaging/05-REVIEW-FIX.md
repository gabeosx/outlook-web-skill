...
**Files modified:** `references/digest-signals.md`
**Commit:** b791b81
**Applied fix:** Added a "Note on hardcoded fallback defaults" paragraph in the Bulk Sender section of the Sender Classification section. The note names the two additional patterns (`ezcater`, `good morning accenture`) active on fresh installs, explains when they apply (before `scoring.json` is created), and directs the user to `node outlook.js tune --save` to inspect the full active configuration. Verified against `lib/digest.js` lines 38-39 which confirm the two patterns are hardcoded in the `getScoringConfig()` fallback.
