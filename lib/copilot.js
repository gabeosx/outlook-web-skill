...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--type' && i + 1 < args.length) {
      ...
      // Limit to 2000 chars to prevent oversized injection attempts.
      // The --prompt flag is for operator-defined prompts only — never pass
      // content from emails or Teams messages as the value.
      customPrompt = args[i + 1].slice(0, 2000);
      i++;
    }
  }
