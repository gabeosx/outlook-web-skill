...
function runAgentBrowser(args, policyFile, opts = {}) {
  ...
  // result.status is null on spawn failure, so we check result.error first
  // to surface a meaningful diagnostic message rather than a blank stderr line.
  if (result.error) {
...
function runBatch(commandArrays, policyFile, opts = {}) {
  ...
  // Detect spawn failure (binary not found, permission denied, etc.)
  if (result.error) {
    log(`batch spawn error: ${result.error.message}`);
    return { stdout: '', stderr: result.error.message, status: 1 };
  }
...
function runEval(script, policyFile) {
  ...
  // Detect spawn failure (binary not found, permission denied, etc.)
  if (result.error) {
    log(`eval spawn error: ${result.error.message}`);
    return { stdout: '', stderr: result.error.message, status: 1 };
  }
