 ...
 * Expects: node outlook.js search "<KQL query>" [--limit N] [--folder <name>]
 *
 * @param {string[]} argv - process.argv
 * @returns {{ query: string, limit: number, folder: string|null }}
 */
 ...
 * @param {string|null} [folder] - Optional folder name (raw CLI value, will be normalized)
 * @returns {any|null} Parsed snapshot JSON, or null on failure
 */
function executeComboboxSearch(query, folder) {
  const resolvedFolder = normalizeFolderName(folder);
  // Eval to dismiss the Office add-in trust DOM modal if present.
  // This modal is rendered by Outlook's React code (not a native window.confirm()),
  // so agent-browser's 'dialog dismiss' (CDP Page.handleJavaScriptDialog) cannot dismiss it.
  // The eval finds a button with text "OK" and clicks it; no-ops if the modal is absent.
  const DISMISS_ADDIN_DIALOG = "(function(){ var b = Array.from(document.querySelectorAll('button')).find(function(el){ return el.textContent.trim() === 'OK'; }); if(b){ b.click(); return 'clicked'; } return null; })()";

  const folderCommands = resolvedFolder ? [
    ['eval', DISMISS_ADDIN_DIALOG],                   // re-dismiss: dialog may re-appear after page events
    ['wait', '300'],
  ...
  const result = runBatch([
    ...
    ['eval', DISMISS_ADDIN_DIALOG],                // dismiss Office add-in trust DOM modal (fires on every load)
    ['wait', '500'],                               // let modal dismiss animation complete
    ...folderCommands,                             // optional folder navigation before search
    ...
    ['eval', "JSON.stringify(Array.from(document.querySelectorAll('[role=option]')).map(el => el.getAttribute('data-convid')))"],  // D-01/D-03: extract Exchange ItemIDs (data-convid attr) while page is live
  ...
  // Phase 3 fix (D-02): Parse eval output (JSON array of IDs) + snapshot text.
  //
  // Live test discovery: agent-browser batch JSON-encodes eval return values.
  // Since the eval calls JSON.stringify() (returning a string), agent-browser
  // double-encodes it: the line in batch stdout looks like:
  //   "[\"AAQk...\",\"AAQk...\"]"
  // (a JSON-encoded string containing a JSON-encoded array).
  //
  // Attribute used: data-convid (not el.id which returns internal React GUIDs).
  // data-convid holds the Exchange ConversationId (AAQk... format).
  // NOTE: This is NOT the message-level ItemId (AAkALg... format) used in the
  // /mail/id/ URL pattern. The read subcommand uses search+click to navigate
  // by data-convid instead of direct URL navigation.
  let ids = [];
  let evalLineEnd = 0;
  // Match a JSON-encoded string that starts with "[  (the double-encoded array)
  const doubleEncodedMatch = cleaned.match(/^"(\[.*?\])"\s*$/m);
  if (doubleEncodedMatch) {
    try {
      const innerJson = JSON.parse(doubleEncodedMatch[0]);  // strips outer quotes, unescapes
      ...
      log('search: eval JSON double-decode failed — falling back to null IDs');
    }
  } else {
    // Fallback: try bare JSON array (in case agent-browser behavior changes)
  ...
  // Snapshot text is everything after the eval output line
