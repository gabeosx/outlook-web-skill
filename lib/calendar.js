...
function runCalendarRead() {
  ...
  // Parse meeting link from MEETING_LINK_EVAL output.
  // The eval returns a JSON-stringified array: e.g. '["https://teams.microsoft.com/..."]'
  // May be double-encoded (like other evals): '"[\"https://...\"]"'
  let meetingLinkFromEval = null;
    ...
    if (doubleMatch) {
      try {
        ...
        if (Array.isArray(arr) && arr.length > 0 && arr[0]) {
          meetingLinkFromEval = String(arr[0]).replace(/[).,]+$/, '');
          break;
        }
  ...
  if (meetingLinkFromEval) {
    log(`calendar-read: meeting link extracted from DOM eval (${meetingLinkFromEval.slice(0, 60)}...)`);
  }
  ...
  // DOM eval takes priority over text-extracted link (href is authoritative over body text parsing)
