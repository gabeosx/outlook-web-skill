...
/**
 * Fetch the meeting link by navigating to the full event view.
 *
 * The popup card body is truncated — Outlook only loads the Teams/Zoom anchor
 * element into the DOM when the full event view is opened via "View event".
 * This function runs a separate batch: re-clicks the event, clicks "View event",
 * waits for the full event panel to load, then evals for the link.
 *
 * Only called when is_online_meeting is true and the popup eval returned null.
 *
 * @param {string} calendarUrl
 * @param {string} clickEval - eval that clicks the target event button
 * @returns {string|null} Teams or Zoom URL, or null if not found
 */
function fetchMeetingLinkViaFullEvent(calendarUrl, clickEval) {
  const viewEventEval = '(function(){' +
    'var btns=Array.from(document.querySelectorAll("button"));' +
    'var btn=btns.find(function(b){' +
    '  var lbl=(b.getAttribute("aria-label")||b.innerText||"").trim();' +
    '  return lbl==="View event";' +
    '});' +
  ...
  const batch = [
    ['open', calendarUrl],
    ['wait', '5000'],
    ['eval', clickEval],         // re-click event button → popup opens
    ['wait', '2000'],            // popup appears
    ['eval', viewEventEval],     // click "View event" → full event body loads
    ['wait', '3000'],            // full event panel renders
    ['eval', MEETING_LINK_EVAL], // extract Teams/Zoom href from full body DOM
  ];

  log('calendar-read: popup body truncated — fetching meeting link via full event view');
  const result = runBatch(batch, POLICY);
  if (result.status !== 0) {
    log('calendar-read: full-event batch failed — meeting link not extracted');
    return null;
  }

  const link = parseMeetingLinkEvalResult(stripContentBoundaries(result.stdout).split('\n'));
  if (link) {
    log(`calendar-read: meeting link found in full event view: ${link.slice(0, 60)}...`);
  } else {
    log('calendar-read: meeting link not found in full event view either');
  }
  return link;
}
...
function runCalendarRead() {
  ...
  // Resolve meeting link with priority: popup DOM eval > text extraction > full event view fallback.
  // The popup card body is truncated, so the Teams URL usually isn't in the popup DOM.
  // fetchMeetingLinkViaFullEvent clicks "View event" which loads the full body with the anchor.
  let meeting_link = meetingLinkFromEval || parsed.meeting_link;
  if (!meeting_link && parsed.is_online_meeting) {
    meeting_link = fetchMeetingLinkViaFullEvent(calendarUrl, clickEval);
  }
