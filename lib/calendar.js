...
// Eval script: click "View event" button in the open popup card.
// Used by --full mode to navigate from the popup to the full event panel,
// which loads the complete body text (including Teams join URL) and attendee list.
const VIEW_EVENT_EVAL = '(function(){' +
  'var btns=Array.from(document.querySelectorAll("button"));' +
  'var btn=btns.find(function(b){' +
  '  return (b.getAttribute("aria-label")||b.innerText||"").trim()==="View event";' +
  '});' +
 ...
 * 2.  Build calendar week-view URL (renders all 7 days in the ARIA tree at once)
 * 3.  Run initial batch (open week view, wait, get url, eval, snapshot)
 * 4.  Session check
 ...
 * 11. Output JSON envelope
 */
function runCalendar() {
  const { days } = parseCalendarArgs(process.argv);
  log(`calendar: starting — days=${days}`);

  // Use Week view URL directly — /calendar/ redirects to Day view (only today's events appear in
  // the ARIA tree). /calendar/view/week renders all 7 days at once. (confirmed: references/outlook-ui.md)
  const calendarUrl = (process.env.OUTLOOK_BASE_URL || '').replace(/\/?$/, '') + '/calendar/view/week';
  log(`calendar: navigating to ${calendarUrl}`);

  // Initial batch: open calendar week view, wait for render, session check, eval (no-op), snapshot
  ...
  for (let week = 0; week < additionalWeeks; week++) {
    ...
    const cleanedNext = stripContentBoundaries(nextWeekResult.stdout);
    const { evalLineEnd: nextEvalEnd } = parseEventIds(cleanedNext);
    const nextSnapshotText = cleanedNext.slice(nextEvalEnd).trim();

    if (!nextSnapshotText) {
      log(`calendar: week ${week + 2} returned empty snapshot — stopping`);
      break;
    }

    const nextRows = extractCalendarRows(nextSnapshotText);
    let nextAdded = 0;
    ...
    log(`calendar: week ${week + 2} added ${nextAdded} new rows`);
  }
 ...
 * Expects: node outlook.js calendar-read <event-id> [--full]
 *
 * --full: navigate to the full event view to retrieve individual attendee list
 * and a reliable meeting link (Teams join URL). Without --full, the popup card
 * is used (faster, but meeting_link is usually null for Teams meetings and
 * attendees is always an empty array).
 *
 * @param {string[]} argv - process.argv
 * @returns {{ eventId: string, full: boolean }}
 */
function parseCalendarReadArgs(argv) {
  ...
  if (!eventId) {
    outputError('calendar-read', 'INVALID_ARGS', 'Usage: node outlook.js calendar-read <event-id> [--full]');
  }
  return { eventId, full };
}
...
function parseCalendarDetailSnapshot(text, eventId) {
  ...
  return {
    ...
    attendee_summary: attendeeCountText, // aggregate count text from popup (e.g. "Accepted 5, Didn't respond 3")
    attendees: [], // popup shows counts only; full attendee list requires --full (View event)
    body_text,
  };
}

/**
 * Parse the full event panel snapshot (opened via "View event") to extract
 * individual attendees and a reliable meeting link.
 *
 * The full event panel loads the complete event body, including any Teams/Zoom
 * join URLs. It also shows attendees grouped by response status (Accepted,
 * Tentative, Declined, None). Parsing is best-effort — ARIA structure varies
 * by Outlook version and account type.
 *
 * Attendee ARIA patterns handled:
 *   heading "Accepted (N)"  → listitem "Name"  (most common)
 *   StaticText "Accepted N" → listitem "Name"  (older layout)
 *   option "Name"                               (some list views)
 *
 * @param {string} text - Full accessibility snapshot of the full event panel
 * @returns {{ attendees: Array<{name: string, response: string}>, meeting_link: string|null, body_text: string }}
 */
function parseFullEventSnapshot(text) {
  const lines = text.split('\n');
  const attendees = [];
  const bodyLines = [];
  let currentResponse = null;

  // Response status section header patterns
  const responseHeaderRe = /(?:heading|StaticText)\s+"(Accepted|Declined|Tentative|None|No response|Didn't respond)(?:\s*\(\d+\))?(?:\s+\d+)?"/i;

  // Name-like listitem/option under a response section
  const attendeeEntryRe = /^\s*-\s+(?:listitem|option)\s+"([^"]{1,100})"/;

  // Generic StaticText (for body accumulation and fallback name detection)
  const staticRe = /^\s*-\s+StaticText\s+"(.+?)"\s*(?:\[.*)?$/;

  // Track whether we are inside a recognised attendee section
  // (stop accumulating names when we hit an unrecognised structural element)
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Response status section header — switch current response bucket
    const headerMatch = line.match(responseHeaderRe);
    if (headerMatch) {
      const raw = headerMatch[1].toLowerCase();
      if (raw === 'accepted') currentResponse = 'accepted';
      else if (raw === 'declined') currentResponse = 'declined';
      else if (raw === 'tentative') currentResponse = 'tentative';
      else currentResponse = 'none';
      inAttendeeSection = true;
      inBody = false;
      continue;
    }

    // Attendee entry (listitem or option under a response section)
    if (inAttendeeSection && currentResponse !== null) {
      const entryMatch = line.match(attendeeEntryRe);
      if (entryMatch) {
        const name = entryMatch[1].trim();
        // Filter out noise: pure numbers, single chars, very short strings
        if (name.length > 1 && !/^\d+$/.test(name)) {
          attendees.push({ name, response: currentResponse });
        }
        continue;
      }

      // A StaticText that looks like a name (no digits-only, reasonable length)
      const sm = line.match(staticRe);
      if (sm) {
        const text2 = sm[1].trim();
        // Short plausible name (2-60 chars, not a heading/sentence)
        if (text2.length >= 2 && text2.length <= 60 && !/[.?!]$/.test(text2) && !/^\d/.test(text2)) {
          attendees.push({ name: text2, response: currentResponse });
          continue;
        }
        // Longer text → this is body content; exit attendee section
      ...
      // Non-text structural element — exit attendee section
      if (!/^\s*-\s+(?:listitem|option|StaticText|img|generic)/.test(line) &&
          !/^\s*$/.test(line)) {
        inAttendeeSection = false;
        inBody = true;
      }
    ...
    // Body text accumulation (after attendee sections)
    if (inBody) {
      const sm = line.match(staticRe);
      if (sm) bodyLines.push(sm[1].trim());
    }
  }
  ...
  return { attendees, meeting_link, body_text };
}
...
function runCalendarRead() {
  const { eventId, full } = parseCalendarReadArgs(process.argv);
  log(`calendar-read: eventId="${eventId.slice(0, 80)}..." full=${full}`);
  ...
  // Batch 1: open calendar -> wait for render -> click event -> wait for popup ->
  ...
  // Default values from popup (fast path)
  let meeting_link = meetingLinkFromEval || parsed.meeting_link;
  let attendees = parsed.attendees; // always [] from popup

  // --full: run second batch to navigate to the full event view.
  // This loads the complete event body (reliable Teams join URL) and the
  // attendee list with individual names and response statuses.
  if (full) {
    log('calendar-read: --full requested — running full event batch');
    const fullBatch = [
      ['open', calendarUrl],
      ['wait', '5000'],              // SPA render
      ['eval', clickEval],           // click event → popup opens
      ['wait', '2000'],              // popup renders
      ['eval', VIEW_EVENT_EVAL],     // click "View event" → full event panel loads
      ['wait', '3000'],              // full event panel renders
      ['eval', MEETING_LINK_EVAL],   // extract Teams/Zoom href from full DOM
      ['snapshot'],                  // full snapshot for attendee parsing (MUST be last D-16)
    ];

    const fullResult = runBatch(fullBatch, POLICY);

    if (fullResult.status !== 0) {
      log(`calendar-read: full event batch failed (status ${fullResult.status}) — returning popup data only`);
    } else {
      const fullCleaned = stripContentBoundaries(fullResult.stdout);
      log(`calendar-read: full event snapshot ${fullCleaned.length} chars`);
      const fullLines = fullCleaned.split('\n');

      // Prefer meeting link from full event DOM eval (reliable — full body loaded)
      const fullMeetingLinkFromEval = parseMeetingLinkEvalResult(fullLines);
      if (fullMeetingLinkFromEval) {
        log(`calendar-read: meeting link found in full event DOM eval: ${fullMeetingLinkFromEval.slice(0, 60)}...`);
        meeting_link = fullMeetingLinkFromEval;
      }

      // Parse attendees and body from full event snapshot
      const fullParsed = parseFullEventSnapshot(fullCleaned);
      if (fullParsed.attendees.length > 0) {
        log(`calendar-read: found ${fullParsed.attendees.length} attendees in full event view`);
        attendees = fullParsed.attendees;
      } else {
        log('calendar-read: no attendees parsed from full event snapshot (ARIA structure may differ)');
      }
      // Fall back to body-text meeting link if DOM eval missed it
      if (!meeting_link && fullParsed.meeting_link) {
        meeting_link = fullParsed.meeting_link;
      }
    }
  }
  ...
  outputOk('calendar-read', {
    ...
    attendee_summary: parsed.attendee_summary,
    attendees,
...
function runCalendarSearch() {
  ...
  // Filter out null start_time — mini-calendar day-number buttons pass extractCalendarRows()
  // year-pattern check but produce no parseable time component.
  const limitedRows = rows.slice(0, limit);
  const results = limitedRows.map(row => parseCalendarEventName(row)).filter(e => e.start_time !== null);
