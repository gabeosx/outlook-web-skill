  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--folder') {
      if (i + 1 < args.length) {
        ...
        // --folder is last arg with no value — warn and exit
        outputError('digest', 'INVALID_ARGS', '--folder requires a value. Usage: node outlook.js digest [--folder <name>]');
      }
    }
  }
...
function runDigest() {
  ...
  const folderCommands = resolvedFolder ? [
    ['eval', DISMISS_ADDIN_DIALOG],  // re-dismiss: dialog may re-appear after get url or other page events
    ['wait', '300'],
  ...
  // folderCommands are injected AFTER get url (session check) and BEFORE eval/snapshot (Pitfall 2)
  const initialBatch = [
    ...
    ...folderCommands,          // folder navigation after session check (if --folder provided)
