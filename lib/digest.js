...
// Urgency keywords for importance scoring (D-15)
// Travel keywords added: travel agency emails (e.g. CWT) can surface as urgent via content
...
// Channel/system senders get a reduced unread bonus (+20 vs +40 for human senders).
// Words that appear in channel/group/system display names — not found in human "Lastname, Firstname" names.
...
// Patterns that indicate automated/bulk email — suppress the unread bonus entirely.
// Bulk newsletters being unread is not a signal of importance.
const BULK_PATTERNS = [
  // Automated sender addresses
  ...
  // Viva Engage / Teams channel notifications
  'started a new conversation', 'new posts from',
  // Marketing emails
  'ezcater',
  // Daily briefing / motivational
...
function runDigest() {
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    const isBulk = BULK_PATTERNS.some(p => searchText.includes(p));
    ...
    const hasChannelWord = fromWords.some(w => CHANNEL_WORDS.has(w));
    ...
    if (!parsed.is_read && !isBulk) {
      const unreadBonus = isHumanSender ? 40 : 20;  // channels get partial credit
