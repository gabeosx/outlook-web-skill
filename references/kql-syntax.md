...
|----------|--------|---------|-------|
| Sender address | `from:<address>` | `from:[REDACTED_EMAIL]` | Matches sender email address |
| Subject | `subject:<word>` | `subject:budget` | Matches words in subject line; use quotes for phrases: `subject:"Q1 budget"` |
| Has attachment | `hasattachment:true` | `hasattachment:true` | Boolean — value required |
...
```bash
node outlook.js search "from:[REDACTED_EMAIL]"
node outlook.js search "subject:budget"
node outlook.js search "hasattachment:true"
...
node outlook.js search "from:[REDACTED_EMAIL] subject:budget"

# Unverified — multiple boolean filters
node outlook.js search "is:unread hasattachment:true"

# Unverified — NOT operator (excludes matching emails)
node outlook.js search "is:unread NOT from:[REDACTED_EMAIL]"

# Unverified — OR operator (must be uppercase)
node outlook.js search "from:[REDACTED_EMAIL] OR from:[REDACTED_EMAIL]"
```

If a compound query returns no results but you expect some, try simplifying to a single operator to confirm which part may not work on this tenant. For example, test `from:[REDACTED_EMAIL]` alone before combining with `is:unread`.
...
node outlook.js search "from:[REDACTED_EMAIL] is:unread"
