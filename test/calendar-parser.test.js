...
describe('parseCalendarEventName — timed recurring event (Teams meeting)', () => {
  ...
  const name = 'Project Planning Sync (daily), 9:00 AM to 9:30 AM, Tuesday, April 14, 2026, By Org, Alpha, Tentative, Recurring event';
  ...
  it('parses subject correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.subject, 'Project Planning Sync (daily)');
  });
  ...
  it('parses organizer with lastname, firstname format', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Alpha');
  });
  ...
  it('id is a non-empty string (composite key)', () => {
    ...
    assert.equal(parsed.subject, 'Project Planning Sync (daily)');
  });
});
...
describe('parseCalendarEventName — timed event with Microsoft Teams Meeting location', () => {
  ...
  const name = 'Team Standup Meeting, 4:30 PM to 4:55 PM, Tuesday, April 14, 2026, Microsoft Teams Meeting, By Org, Beta, Tentative, Recurring event';

  it('parses subject correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.subject, 'Team Standup Meeting');
  });
  ...
  it('parses organizer with lastname, firstname format', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Beta');
  });
...
describe('parseCalendarEventName — timed event with Zoom URL as location', () => {
  ...
  const name = 'Vendor Strategy Session?, 12:30 PM to 2:00 PM, Thursday, April 16, 2026, https://company.zoom.us/j/12345678901?pwd=..., By Org Gamma, Tentative';

  it('parses subject correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.subject, 'Vendor Strategy Session?');
  });
  ...
  it('parses organizer as single name (no comma)', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org Gamma');
  });
...
describe('parseCalendarEventName — timed event with physical location (multi-part address)', () => {
  ...
  const name = 'Team Networking Happy Hour, 5:30 PM to 7:00 PM, Thursday, April 16, 2026, 123 Main Street, Anytown, NY 10001, By Org, Delta, Tentative';

  it('parses location including street address parts', () => {
    const result = parseCalendarEventName(name);
    // Location should include all address parts before "By organizer"
    assert.ok(result.location && result.location.includes('123 Main Street'), `location was: ${result.location}`);
  });

  it('parses organizer with lastname, firstname format', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Delta');
  });
...
describe('parseCalendarEventName — exception to recurring event', () => {
  ...
  const name = '[Internal Forum Talk [69]]: ..., 10:00 AM to 11:00 AM, Thursday, April 16, 2026, Microsoft Teams Meeting, By Org, Epsilon, Tentative, Exception to recurring event';
  ...
  it('parses organizer correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Epsilon');
  });
...
describe('parseCalendarEventName — all-day single-day event with organizer', () => {
  ...
  const name = 'Organizer - working out of country, all day event, Sunday, April 12, 2026, By Org, Zeta, Free, Recurring event';
  ...
  it('parses organizer with lastname, firstname format', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Zeta');
  });
...
describe('parseCalendarEventName — all-day event with location and organizer', () => {
  ...
  const name = 'Team Social Event, all day event, Thursday, April 16, 2026, Company HQ - Floor 65, By Org, Eta, Free, Recurring event';

  it('parses location correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.location, 'Company HQ - Floor 65');
  });

  it('parses organizer with lastname, firstname format', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, 'Org, Eta');
  });
...
describe('extractCalendarRows', () => {
  ...
  const snapshotText = `
    - main "calendar view, Week of April 14 to 18, 2026"
      ...
      - button "Next week"
      - button "Today"
      - button "Project Planning Sync (daily), 9:00 AM to 9:30 AM, Tuesday, April 14, 2026, By Org, Alpha, Tentative, Recurring event"
      - button "Team Standup Meeting, 4:30 PM to 4:55 PM, Tuesday, April 14, 2026, Microsoft Teams Meeting, By Org, Beta, Tentative, Recurring event"
  ...
  it('first row is the timed AIG event', () => {
    const rows = extractCalendarRows(snapshotText);
    assert.ok(rows[0].startsWith('Project Planning Sync (daily)'), `got: ${rows[0].slice(0, 60)}`);
  });
...
describe('extractMeetingLink', () => {
  ...
  it('extracts Zoom URL', () => {
    const link = extractMeetingLink('Join Zoom Meeting https://company.zoom.us/j/12345678901?pwd=abc123 Thanks');
    assert.ok(link, 'should return a link');
