...
describe('parseCalendarEventName — timed event with Zoom URL as location', () => {
  ...
  const name = 'Hold for Hitachi + Okta + [REDACTED_COMPANY]?, 12:30 PM to 2:00 PM, Thursday, April 16, 2026, https://okta.zoom.us/j/95577737751?pwd=..., By [REDACTED_PERSON], Tentative';

  it('parses subject correctly', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.subject, 'Hold for Hitachi + Okta + [REDACTED_COMPANY]?');
  });
  ...
  it('parses organizer as single name (no comma)', () => {
    const result = parseCalendarEventName(name);
    assert.equal(result.organizer, '[REDACTED_PERSON]');
  });
